...
const SkillsForm = ({ formData = {}, setFormData, userId }) => {
  ...
  useEffect(() => {
    console.log('SkillsForm received formData:', formData);
    console.log('SkillsForm received userId:', userId);
  }, [formData, userId]);
  ...
  const addSkill = (category) => {
    ...
    if (!currentSkills.includes(skill)) {
      ...
      setFormData({ ...formData, [category]: newSkills }); // Update parent state directly
      console.log(`Added ${skill} to ${category}:`, newSkills);
    }
    setInputValues((prev) => ({ ...prev, [category]: '' }));
  };
  ...
  const removeSkill = (category, skillToRemove) => {
    ...
    setFormData({ ...formData, [category]: newSkills }); // Update parent state directly
    //console.log(`Removed ${skillToRemove} from ${category}:`, newSkills);
  };

  const getSkillsArray = (category) => {
    const skills = formData[category] ? formData[category].split(',').map((s) => s.trim()).filter(Boolean) : [];
    console.log(`${category} skills array:`, skills);
    return skills;
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      await setDoc(doc(db, 'skills', userId), skillsData, { merge: true });

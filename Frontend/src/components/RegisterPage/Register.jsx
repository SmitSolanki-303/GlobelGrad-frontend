...
function Register() {
  ...
  const [authError, setAuthError] = useState(null); // To display authentication errors
  ...
  const saveUserData = async (user) => {
    try {
      ...
      alert("User data saved successfully to Firebase with ID: " + docRef.id);
      return docRef.id;
    } catch (error) {
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      await saveUserData(user); // Save data after successful sign-up
  ...
  const handleCompleteRegistration = async () => {
    try {
      if (!savedDocId) {
        // If "Save" hasn't been clicked, sign up and save data
        await handleSignUp();
      }
      navigate("/protfolio-form");
    } catch (error) {
                  ...
                  <div className="space-y-6">
                    <div className="bg-blue-50 border border-blue-100 rounded-lg p-6">
                      ...
                      <p className="text-blue-700 mb-4">
                        Please review your information before finalizing your registration.
                      </p>
                      <div className="flex items-center mb-4">
                        <FaCheckCircle className="text-green-500 mr-2" />
                        <span className="text-gray-700">Personal information collected</span>
